...
describe('floodFill', () => {
  it('should throw error if seed point is out of bounds', () => {
    ...
    expect(() => floodFill(image, { x: -1, y: 5 }, predicate)).toThrow(
      'Seed point out of image bounds'
    );
    expect(() => floodFill(image, { x: 5, y: -1 }, predicate)).toThrow(
      'Seed point out of image bounds'
    );
    expect(() => floodFill(image, { x: 10, y: 5 }, predicate)).toThrow(
      'Seed point out of image bounds'
    );
    expect(() => floodFill(image, { x: 5, y: 10 }, predicate)).toThrow(
      'Seed point out of image bounds'
    );
  });
  ...
  it('should respect maxPixels limit', () => {
    ...
    expect(() => floodFill(image, seed, predicate, config)).toThrow(
      'Region too large: exceeded 100 pixels'
    );
  });
