...
interface UIState extends UIContextState {
  ...
  addFrame: (
    frame: Partial<FrameData> &
      Pick<FrameData, 'x' | 'y' | 'width' | 'height' | 'rotation'>
  ) => void;
...
const findFrameByIdHelper = (
  framesByPage: Record<string, FrameData[]>,
  frameId: string
): { frame: FrameData; pageId: string } | undefined => {
    ...
    immer((set, get) => ({
      ...
      addFrame: (frame) =>
        set((state) => {
          ...
          console.log(
            '[UIStore] Created frame has imageData:',
            !!newFrame.imageData
          );
          ...
          newFrame.x = Math.max(
            0,
            Math.min(newFrame.x, state.currentPage.width)
          );
          newFrame.y = Math.max(
            0,
            Math.min(newFrame.y, state.currentPage.height)
          );
          ...
          for (const [pageId, frames] of Object.entries(state.framesByPage)) {
            ...
            if (frameIndex !== -1) {
              ...
              if (state.currentPage) {
                frame.x = Math.max(
                  0,
                  Math.min(frame.x, state.currentPage.width)
                );
                frame.y = Math.max(
                  0,
                  Math.min(frame.y, state.currentPage.height)
                );
              }
      ...
      renameFrame: (id, label) =>
        set((state) => {
          ...
          if (result) {
            const frameIndex = state.framesByPage[result.pageId].findIndex(
              (f) => f.id === id
            );
            if (frameIndex !== -1) {
      ...
      translateFrameRelative: (id, vector) =>
        set((state) => {
          const result = findFrameByIdHelper(state.framesByPage, id);
      ...
      setOrientation: (id, orientation) =>
        set((state) => {
          ...
          if (result) {
            ...
            if (frameIndex !== -1) {
              state.framesByPage[result.pageId][frameIndex].orientation =
                orientation;
            }
          }
        }),

      updatePage: (updates, imagePath) =>
        set((state) => {
          if (!imagePath) {
