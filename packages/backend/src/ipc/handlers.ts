...
export function setupIpcHandlers(workspaceService: WorkspaceService) {
  ...
  ipcMain.handle(
    ...
    async (event, path: string, options?: LoadDirectoryOptions) => {
      console.log(
        '[IPC] workspace:loadDirectory called with path:',
        path,
        'options:',
        options
      );
    ...
    async (
      ...
      try {
        ...
        return {
          success: false,
          error:
            error instanceof Error ? error.message : 'Failed to generate frame',
        };
      }
    }
  );
  ...
  ipcMain.handle(
    ...
    async (event, frameId: string, updates: Partial<FrameData>) => {
      console.log('[IPC] workspace:updateFrame called for frame:', frameId);

      try {
        const updatedFrame = await workspaceService.updateFrame(
          frameId,
          updates
        );
  ...
  ipcMain.handle('workspace:saveFrame', async (event, frameData: FrameData) => {
    ...
    try {
      ...
      const defaultFilename =
        workspaceService.getSanitizedFrameFilename(frameData);
      ...
      const savedPath = await workspaceService.saveFrameToPath(
        frameData,
        result.filePath
      );
  ...
  ipcMain.handle('workspace:rotateFrame', (event, frameData: FrameData) => {
  ...
  ipcMain.handle(
    ...
    async (event, directory: string, filenames: string[]) => {
      console.log(
        '[IPC] workspace:checkFilesExist called with',
        filenames.length,
        'files'
      );
      ...
      try {
        ...
        return {
          success: false,
          error:
            error instanceof Error ? error.message : 'Failed to check files',
        };
      }
    }
  );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      console.log(
        '[IPC] workspace:saveAllFrames called with',
        frames.length,
        'frames'
      );
      ...
      try {
        ...
        return {
          success: false,
          error:
            error instanceof Error ? error.message : 'Failed to save frames',
        };
      }
    }
  );
